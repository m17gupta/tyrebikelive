// custom-translations.ts — standalone version without @payloadcms/translations
// NestedKeysStripped has been replaced with a simple recursive key flattener

type NestedObject = { [key: string]: string | NestedObject };

type NestedKeys<T extends NestedObject, Prefix extends string = ""> = {
  [K in keyof T]: T[K] extends NestedObject
    ? NestedKeys<T[K], `${Prefix}${K & string}:`>
    : `${Prefix}${K & string}`;
}[keyof T];

export const customTranslationsObject = {
  en: {
    custom: {
      inPostMessage:
        "InPost charges fees for generating shipments. Check your balance, customer data and dimensions before creating a shipment.",
      generating: "Generating...",
      downloadLabel: "Download label",
      downloadingLabel: "Downloading...",
      createPackage: "Create package",
      resetPackage: "Reset package",
      resetPassword: "Reset password (enter email in form)",
      resetPasswordSuccess: "Password reset link sent to email",
      resetPasswordError: "Error resetting password",
    },
    adminDashboard: {
      linkTitle: "Dashboard",
      searchNoResults: "No results",
      search: "Search collections...",
      addProduct: "Add product",
      totalRevenue: "Total revenue",
      totalOrders: "Total orders",
      rangedRevenue: "Revenue in period",
      rangedOrders: "Orders in period",
      "from-last-month": "from last month",
      overview: "Overview",
      revenue: "Revenue",
      orders: "Orders",
      columns: "Columns",
      jan: "January",
      feb: "February",
      mar: "March",
      apr: "April",
      may: "May",
      jun: "June",
      jul: "July",
      aug: "August",
      sep: "September",
      oct: "October",
      nov: "November",
      dec: "December",
      filterEmails: "Filter emails...",
      recentOrders: "Recent orders",
      date: "Date",
      status: "Status",
      email: "Email",
      amount: "Amount",
      pending: "Pending",
      paid: "Paid",
      unpaid: "Unpaid",
      processing: "Processing",
      shipped: "Shipped",
      completed: "Completed",
      cancelled: "Cancelled",
      returned: "Returned",
      previous: "Previous",
      next: "Next",
      actions: "Actions",
      copyPaymentID: "Copy payment ID",
      viewCustomer: "View customer",
      viewOrder: "View order",
      openMenu: "Open menu",
    },
  },
  zh: {
    custom: {
      inPostMessage:
        "InPost 會收取產生貨件的費用。請在建立貨件前檢查您的餘額、客戶資料及尺寸。",
      generating: "產生中...",
      downloadLabel: "下載標籤",
      downloadingLabel: "下載中...",
      createPackage: "建立包裹",
      resetPackage: "重設包裹",
      resetPassword: "重設密碼（請在表單中輸入電子郵件）",
      resetPasswordSuccess: "密碼重設連結已寄至您的電子郵件",
      resetPasswordError: "重設密碼時發生錯誤",
    },
    adminDashboard: {
      linkTitle: "控制台",
      searchNoResults: "沒有結果",
      search: "搜尋集合...",
      addProduct: "新增商品",
      totalRevenue: "總收入",
      totalOrders: "總訂單",
      rangedRevenue: "期間收入",
      rangedOrders: "期間訂單",
      "from-last-month": "較上個月",
      overview: "概覽",
      revenue: "收入",
      orders: "訂單",
      columns: "欄位",
      jan: "一月",
      feb: "二月",
      mar: "三月",
      apr: "四月",
      may: "五月",
      jun: "六月",
      jul: "七月",
      aug: "八月",
      sep: "九月",
      oct: "十月",
      nov: "十一月",
      dec: "十二月",
      filterEmails: "篩選電子郵件...",
      recentOrders: "近期訂單",
      date: "日期",
      status: "狀態",
      email: "電子郵件",
      amount: "金額",
      pending: "待處理",
      paid: "已付款",
      unpaid: "未付款",
      processing: "處理中",
      shipped: "已出貨",
      completed: "已完成",
      cancelled: "已取消",
      returned: "已退貨",
      previous: "上一頁",
      next: "下一頁",
      actions: "操作",
      copyPaymentID: "複製付款 ID",
      viewCustomer: "查看客戶",
      viewOrder: "查看訂單",
      openMenu: "開啟選單",
    },
  },
};

export type CustomTranslationsObject = typeof customTranslationsObject.en;
export type CustomTranslationsKeys = NestedKeys<CustomTranslationsObject>;
